...
static void dma_user_callback(const struct device *dma_dev, void *arg, uint32_t id, int status)
{
...
static int test_loop(const struct device *dma)
{
	...
	/* Run the loop test on each channel one by one */
	for (int i = 0; i < NUM_CHANNELS; i++) {
		...
		for (int j = 0; j < TRANSFER_LOOPS; j++) {
			TC_PRINT("RX data Loop %d\n", j);
			if (memcmp(tx_data, rx_data[j], CONFIG_DMA_LOOP_TRANSFER_SIZE)) {
				return TC_FAIL;
			}
		}
	}
...
static int test_loop_suspend_resume(const struct device *dma)
{
	...
	/* Run the suspend/resume test on each channel one by one */
	for (int i = 0; i < NUM_CHANNELS; i++) {
		...
		res = dma_resume(dma, i);
		TC_PRINT("Resumed transfers\n");
		if (res != 0) {
			TC_PRINT("ERROR: resume failed, channel %d, result %d", i, res);
			if (dma_stop(dma, i)) {
				TC_PRINT("ERROR: transfer stop\n");
			}
			return TC_FAIL;
...
{
	enum pm_device_state init_state =
		pm_device_on_power_domain(dma) ? PM_DEVICE_STATE_OFF : PM_DEVICE_STATE_SUSPENDED;
	...
	/* Run the start/stop test on each channel one by one */
	for (int i = 0; i < NUM_CHANNELS; i++) {
